...
| Токен | Значение |
|---|---|
| `30m`, `1h`, `1.5h`, `1h 30m`, `t2h` | оценка времени |
| `1h/2h` | потрачено / оценка (потраченное пишется на сегодня) |
| `+Проект` | проект по названию: самый длинный префикс, полное название бьёт частичное, одно слово матчит название без пробелов (`+SomePro` → «Some Pro»); архивные и скрытые проекты не участвуют |
| `#тег` | тег; несуществующий создаётся; `#123` в начале заголовка — не тег (это ссылка на issue); `#today` зарезервирован |
...
(N от 1 до 999; `N=1` схлопывается в обычный пресет). Для `@every monday` и
`@every 15th` `startDate` ставится на ближайшее подходящее число.
...
- при `--backlog` разобранный `@срок` игнорируется (задача в бэклоге по смыслу
  не запланирована).
