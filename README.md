...
./sp bulk [<id...>] [--project P] [--tag T] [--overdue] [--search TEXT] [--done] [--all] \
          ...
          [--move-project P] [--complete|--reopen] [--dry-run] [--yes]
                               ...
                               # --all — отдельный селектор (все задачи, включая сделанные);
                               # всё пишется одним батчем; --dry-run только показывает выборку,
                               # при >10 задачах спрашивает подтверждение (--yes отключает).
                               # --due "" = --clear-due; --due идёт через планировщик (LP,
                               # прошедший день запрещён), --clear-due — через HSX (чистит
                               # и планировщик). --due/--clear-due/--tag-add/--tag-rm не
                               # трогают подзадачи (пропуск с пометкой в stderr).
                               # Если между выборкой и записью совпадений стало БОЛЬШЕ
                               # (файл изменился), запись отменяется — нужно перезапустить
